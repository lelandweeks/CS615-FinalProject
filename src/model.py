...
class MinecraftCNN(nn.Module):
    def __init__(self, in_channels=3, num_kernels=1, kernel_size=3, num_conv_layers=1, num_classes=60):
        ...
        conv_width  = (img_width - kernel_size) / CONV_STRIDE + 1
        conv_height = (img_height - kernel_size) / CONV_STRIDE + 1
        ...
        for i in range(num_conv_layers):
            ...
            layers.append(nn.Conv2d(in_channels=in_channels, out_channels=num_kernels, kernel_size=kernel_size, stride=CONV_STRIDE, padding=0))
        ...
        # calculation: 159x89 -> 1x14,191 for 1 conv layer
        layers.append(nn.Flatten())

        # fully connected layer to output class scores
        # input size = num_kernels * height * width
        # calculation: 1 * 159 * 89 = 14,191 for 1 conv layer
